...
async function runPhase5Audit() {
  ...
  const firstChapterPlan = reconciled.reconciledPlan.sumario[0];
  assert(Boolean(firstChapterPlan && firstChapterPlan.estimativaPalavras >= 1200), 'Adjusts chapter word estimate up to min boundary');
  if (!firstChapterPlan) throw new Error('Reconciliation did not return the first chapter.');

  // 3. Sub-block Chapter Planning Tests
  console.log('\n--- 3. Sub-block Chapter Planning Tests ---');
  const detailed = buildChapterSectionPlan(firstChapterPlan, validConfig.sanitizedMetadata!);
  assert(detailed.sections.length >= 2, 'Divides chapter into sub-block working sections (>= 2 sections)');
  const firstSection = detailed.sections[0];
  assert(Boolean(firstSection && firstSection.estimativaPalavras >= 600 && firstSection.estimativaPalavras <= 1200), 'Sub-block word estimates stay in optimal 600-1200 range');
